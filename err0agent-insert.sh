#!/bin/bash
#
# err0agent-insert.sh - Insert error codes using err0.io agent (production)
#
# Usage:
#   ./err0agent-insert.sh <token-file> <project-directory>
#
# Example:
#   ./err0agent-insert.sh tokens/err0-django-*.json django
#
...
docker run --rm --network="$DOCKER_NETWORK" \
    ...
    "$DOCKER_IMAGE" \
    /usr/local/bin/err0.sh --token "$DOCKER_MOUNT/$1" --insert "$DOCKER_MOUNT/$2"
