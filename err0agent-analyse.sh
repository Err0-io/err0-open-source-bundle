#!/bin/bash
#
# err0agent-analyse.sh - Analyze error codes using err0.io agent (production)
#
# Usage:
#   ./err0agent-analyse.sh <token-file> <project-directory> [--check|--dirty]
#
# Examples:
#   ./err0agent-analyse.sh tokens/err0-django-*.json django --check
#   ./err0agent-analyse.sh tokens/err0-django-*.json django --dirty
#
# Environment Variables:
#   ERR0_DOCKER_IMAGE_LATEST  - Docker image to use (default: err0io/agent:latest)
...
DOCKER_IMAGE="${ERR0_DOCKER_IMAGE_LATEST:-err0io/agent:latest}"
DOCKER_NETWORK="${ERR0_DOCKER_NETWORK:-host}"
DOCKER_MOUNT="${ERR0_DOCKER_MOUNT:-/mnt}"

# Check arguments
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: $0 <token-file> <project-directory> [--check|--dirty]" >&2
    echo "" >&2
    echo "Examples:" >&2
    echo "  $0 tokens/err0-django-*.json django --check" >&2
    echo "  $0 tokens/err0-django-*.json django --dirty" >&2
    echo "" >&2
...
docker run --rm --network="$DOCKER_NETWORK" \
    ...
    "$DOCKER_IMAGE" \
    /usr/local/bin/err0.sh --token "$DOCKER_MOUNT/$1" --analyse --check "$DOCKER_MOUNT/$2"
